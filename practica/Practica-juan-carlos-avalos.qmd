...
9.  ¿Tienen todos los barrios los mismos metros cuadrados de media? ¿Con que test lo comprobarías?
    ...
    ```{r}
    aov( mean ~ Neighbourhood, data=df_madrid_mean_neighbourhood)
    ```

    ```{r}
    boxplot(mean ~ Neighbourhood, data=df_madrid_mean_neighbourhood)
    ```

    ```{r}
    aov( Square.Meters ~ Neighbourhood, data=df_madrid_without_neighbourhood_with_all_na_sm)
    ```

    ```{r}
    cat(paste("Por lo que pudimos ver, las medias NO son similares todas las medias. El test de ANOVA NO nos brinda ningún resultado significado"))
    ```
